...
namespace ControllerTest {
	...
	public class Model_RaceData {
		...
		public void AddItem_ListShouldContainThatItem() {
			var points = new ParticipantPoints() {Name = "Tester1", Points = 5, Contestor = _participants[0]};
			var time = new ParticipantTime() { Name = "Tester1", Time = DateTime.Now.TimeOfDay, Contestor = _participants[0]};
			var sectionTime = new ParticipantSectionTime() {
				Name = "Tester1", Section = new Section(SectionTypes.StartGrid), Contestor = _participants[0], Time = DateTime.Now.TimeOfDay
			};
			var overtaken = new ParticipantsPassing("Tester1", "Tester2") {Contestor = _participants[0]};
			var brokenDown = new ParticipantBreakDown() {Name = "Tester1", Count = 1, Contestor = _participants[0]};
		...
		public void BestParticipant_Points() {
			var expected = new ParticipantPoints() {Name = "Tester1", Points = 5, Contestor = _participants[0]};
			var test = new ParticipantPoints() {Name = "Tester2", Points = 3, Contestor = _participants[1]};
		...
		public void BestParticipant_Overtaken() {
			_participantsOvertaken.AddToList(new ParticipantsPassing("Tester1", "Tester2") {Contestor = _participants[0]});
			_participantsOvertaken.AddToList(new ParticipantsPassing("Tester2", "Tester1") {Contestor = _participants[1]});
			_participantsOvertaken.AddToList(new ParticipantsPassing("Tester1", "Tester2") {Contestor = _participants[0]});
		...
		public void BestParticipant_Time() {
			_participantTime.AddToList(new ParticipantTime() { Name = "Tester1", Time = new TimeSpan(0, 5, 30), Contestor = _participants[0]});
			_participantTime.AddToList(new ParticipantTime() { Name = "Tester2", Time = new TimeSpan(0, 7, 45), Contestor = _participants[1]});
			var expected = _participants[0].Name;
		...
		public void BestParticipant_BrokenDown() {
			_particpantTimesBrokenDown.AddToList(new ParticipantBreakDown() {Name = "Tester1", Count = 1, Contestor = _participants[0]});
			_particpantTimesBrokenDown.AddToList(new ParticipantBreakDown() {Name = "Tester2", Count = 2, Contestor = _participants[1]});
			_particpantTimesBrokenDown.AddToList(new ParticipantBreakDown() {Name = "Tester2", Count = 1, Contestor = _participants[1]});
			var expected = _participants[0].Name;
